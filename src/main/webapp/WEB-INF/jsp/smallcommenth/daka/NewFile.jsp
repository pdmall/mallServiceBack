...
<body class="no-skin">
					...
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="padding-left:20px;"><a href="fhsms/list.do?TYPE=1"><span class="label label-success arrowed-right arrowed-in">收信箱</span></a></td>
								<td><a href="fhsms/list.do?TYPE=2"><span class="label label- arrowed-right arrowed-in">发信箱</span></a></td>
							</tr>
						</table>
						<!-- 检索  -->
	...
	<script type="text/javascript">
		...
		function sendFhsms(username){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = 'http://localhost:8080/fhsms/goAdd.do?username='+username;
			 diag.Width = 660;
			 diag.Height = 444;
		...
		function del(ztid,STATUS,type,Id,SANME_ID){
			bootbox.confirm("确定要删除吗?", function(result) {
				...
				if(result) {
					top.jzts();
					var url = "http://localhost:8080/fhsms/delete.do?FHSMS_ID="+Id+"&tm="+new Date().getTime();
		...
		function viewx(ztid,STATUS,type,Id,SANME_ID){
			if(type == "1" && STATUS == '2' && $("#"+ztid).html() == '<span class="label label-important arrowed-in">未读</span>'){
			 ...
			 diag.Drag=true;
			 diag.Title ="站内信";
			 diag.URL = 'http://localhost:8080/fhsms/goView.do?FHSMS_ID='+Id+'&TYPE='+type+'&SANME_ID='+SANME_ID+'&STATUS='+STATUS;
			 diag.Width = 600;
			 diag.Height = 460;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: 'http://localhost:8080/fhsms/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		function viewUser(USERNAME){
			if('admin' == USERNAME){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = 'http://localhost:8080/user/view.do?USERNAME='+USERNAME;
			 diag.Width = 469;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
