								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center' style="width: 100px;">{{	item.name }}</td>
										
											<td class='center'style="width: 120px;">{{	item.phone }}</td>
											<td class='center'style="width: 330px;">{{	item.site }}</td>
										
											<td class='center' style="width: 300px;">{{item.message}} </td>
											<td class='center ' style="width: 100px;  ">{{ item.cost }}</td>
											<td class='center ' style="width: 80px;  ">
												<span v-if="item.status==1">未接</span>
												<span v-else-if="item.status==2">已接</span>
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>band/getbandlist?page="+this.page;
	         ...
	         methods:{
	        	 ...
	        	 <!--添加用户-->
	      		add:function(){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="增加";
	    			 diag.URL = '<%=basePath%>page/weixiao/usedadd';
	    			 diag.Width = 600;
	    			 diag.Height = 555;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	         	...
	         	<!--修改弹出-->
	            edit:function(item){
	            	 localStorage.setItem("zjaid",item.id);	            	 
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="编辑";
	    			 diag.URL = '<%=basePath%>page/pdt/product/advertisingupdete';
	    			 diag.Width = 600;
	    			 diag.Height = 555;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/band/deletebands";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {id:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>band/getbandlist?page="+this.cur;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>band/getbandlist?page="+this.selecteType;
		          ...
		          changeIsPause:function(selected){
		           ...
		           makeAll:function(){
   					...
   					var url = "<%=basePath%>/advertising/deleteadvertisings";
   					$.ajax({
							type: "POST",
							url: url,
