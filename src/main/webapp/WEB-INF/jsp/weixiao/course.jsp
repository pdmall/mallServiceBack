							...
							<table id="simple-table"
								...
								<thead>
									<tr>
										...
										<th class="center">老师介绍</th>
										<th class="center">课程大纲</th>
								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center' style="width: 230px;">{{	item.title }}</td>
										
											<td class='center'>{{	item.type }}</td>
											<td class='center'>{{	item.time }}</td>
											<td class='center'>{{	item.hour }}</td>
											<td class='center'>{{	item.number }}</td>
											<td class='center'>{{	item.cost/100 }}</td>
											<td class='center'><a @click="photo(item.photo)" >图片</a></td>
											<td class='center'>{{	item.intro }}</td>
											<td class='center ' style="width: 200px;  ">{{ item.teacherintro }}</td>
											<td class='center'><a @click="outline(item.id)" >详情</a></td>
											<td class='center'><a @click="estimate(item.id)" >评价</a></td>
											<td class='center'><a @click="applylist(item.id)" >{{item.numbers}}</a></td>
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>course/getcourselist?page="+this.page;
	         ...
	         methods:{
	        	 ...
	        	 <!--添加用户-->
	      		add:function(){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="增加";
	    			 diag.URL = '/page/weixiao/courseadd';
	    			 diag.Width = 800;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	         	...
	         	<!--修改弹出-->
	            edit:function(item){
	            	 localStorage.setItem("wxcourse",item.id);	            	 
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="编辑";
	    			 diag.URL = '/page/weixiao/courseupdate';
	    			 diag.Width = 800;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            outline:function(item){
	            	 ...
	            	 localStorage.setItem("wxoutline",item);	            	 
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="课程大纲";
	    			 diag.URL = '/page/weixiao/outline';
	    			 diag.Width = 800;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            estimate:function(item){
	            	 ...
	            	 localStorage.setItem("wxestimate",item);	            	 
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="课程大纲";
	    			 diag.URL = '/page/weixiao/estimate';
	    			 diag.Width = 800;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            applylist:function(item){
	            	
	            	 localStorage.setItem("wxapplylist",item);	            	 
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="课程大纲";
	    			 diag.URL = '/page/weixiao/applylist';
	    			 diag.Width = 800;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/course/deletecourse";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {id:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>course/getcourselist?page="+this.cur;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>course/getcourselist?page="+this.selecteType;
		          ...
		          changeIsPause:function(selected){
		           ...
		           makeAll:function(){
   					...
   					var url = "<%=basePath%>/course/deletecourses";
   					$.ajax({
							type: "POST",
							url: url,
