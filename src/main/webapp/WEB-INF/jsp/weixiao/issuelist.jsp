								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center'>{{	item.school }}</td>
											<td class='center'>{{	item.intro }}</td>
											<td class='center'>{{	item.cost }}</td>
											<td class='center'><a @click="photo(item.id)" >认证</a></td>
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>issue/getissuelists?page="+this.page;
	         ...
	         methods:{
	        	 photo:function(photo){
	            	 ...
	            	 localStorage.setItem("wxissue",photo);
	            	 top.jzts();
	        	 ...
	        	 <!--添加用户-->
	      		add:function(){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="增加";
	    			 diag.URL = '<%=basePath%>page/weixiao/issueadd';
	    			 diag.Width = 800;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/issue/deleteeissue";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {id:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>issue/getissuelists?page="+this.cur;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>issue/getissuelists?page="+this.selecteType;
