							...
							<table id="simple-table"
								...
								<thead>
									<tr>
										...
										<th class="center hidden">创建时间</th>
								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center' style="width: 230px;">{{	item.type }}</td>
										
											<td class='center'>{{	item.site }}</td>
											<td class='center'>{{	item.uphone }}</td>
											<td class='center'>{{	item.gift }}</td>
											<td class='center'>{{	item.intro }}</td>
											<td class='center'>{{	item.time }}</td>
											<td class='center'>{{	item.cost }}</td>
											<td class='center'><a @click="photo(item.photo)" >图片</a></td>
											<td class='center ' style="width: 200px;  ">
												<span v-if="item.status==1"><a @click="getstatus(item.id,2);">空闲</a></span>
												<span v-else-if="item.status==2"><a @click="getstatus(item.id,1)">有人</a></span>
												
											</td>
											
											<td class='center'>{{item.name}}</td>
											<td class='center ' style="width: 200px;  ">{{ item.phone }}</td>
											<td class='center ' style="width: 50px;  ">{{ item.number }}</td>
											
											<td class='center hidden' style="width: 200px;">
													<div class="col-sm-9">
														 <label v-if="item.status==0"> <input name="switch-field-1" class="ace ace-switch ace-switch-4 btn-flat"  type="checkbox"   @click="change(item.id,item.status)"> 
															<span class="lbl"></span> 
														</label> 
														 <label v-else> <input name="switch-field-1" class="ace ace-switch ace-switch-4 btn-flat" checked="checked"  type="checkbox"   @click="change(item.id,item.status)"> 
															<span class="lbl"></span> 
														</label> 
													</div>
												</td>
								...
								<table style="width: 100%;">
									<tr>
										<td style="vertical-align: top; width: 140px;"><a
											class="btn btn-xs btn-success" title="增加" @click="add(  )">
												增加 </a> </td>
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>recrea/getrecreationlist?page="+this.page;
	         ...
	         methods:{
	        	 ...
	        	 getstatus:function(id,status){
	        		...
	        		var url ="<%=basePath%>/recrea/updaterecreation";	 
	        		$.ajax({
						type: "POST",
						url: url,
				    	data: {
	        	 ...
	        	 photo:function(photo){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="查看";
	    			 diag.URL = '/page/pdt/product/photo';
	    			 diag.Width = 450;
	    			 diag.Height = 450;
	        	 ...
	        	 <!--添加用户-->
	      		add:function(){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="增加";
	    			 diag.URL = '/page/weixiao/recreationadd';
	    			 diag.Width = 900;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	         	...
	         	<!--修改弹出-->
	            edit:function(item){
	            	 localStorage.setItem("recrea",item.id);	            	 
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="编辑";
	    			 diag.URL = '/page/weixiao/recreationupdate';
	    			 diag.Width = 900;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/recrea/deleterecreation";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {id:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>recrea/getrecreationlist?page="+this.cur;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>recrea/getrecreationlist?page="+this.selecteType;
