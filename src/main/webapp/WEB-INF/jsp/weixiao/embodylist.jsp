								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center'>{{	item.money/100 }}</td>
											
											<td class='center ' style="width: 200px;  ">
												<span v-if="item.status==1">未处理</span>
												<span v-else-if="item.status==2">已处理</span>
											...
											<td class="center ">
												<div class="hidden-sm hidden-xs btn-group" id="dsdssd">
													 ...
													 <a v-if="item.status==1"
														@click="updates( item.id,2 )"> <i
														class="bigger-120" title="处理">处理</i>
													</a>
													 <a   v-if="item.status==2"  class="btn btn-xs btn-danger"
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>embody/getembodylist?page="+this.page;
	         ...
	         methods:{
	        	 ...
	        	 updates:function(id,status){
	        		
	        		var url ="<%=basePath%>/embody/updateembody";	 
	        		$.ajax({
						type: "POST",
						url: url,
				    	data: {
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/embody/deleteembody";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {id:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>embody/getembodylist?page="+this.cur;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>embody/getembodylist?page="+this.selecteType;
