							...
							<tr>
								<td>
									...
									<select   id="type"  style="width:80px;">
										...
										<input class="nav-search-input" autocomplete="off" id="message" type="text" name="keywords" placeholder="这里输入关键词" />
										<a class="btn btn-xs btn-danger" onclick="getuser()">搜索</a>
									</span>
									</div>
								</td>
								...
								<td> <a href="javascript:;" class="file">导入D盘用户文件<input type="file" name="fileString" id=""></a></td>
							</tr>
						</table>
						<!-- 检索  -->
									...
									<c:forEach items="${userLists}" var="user" varStatus="vs">
										<tr>
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${user.userId }" id="${user.email }" alt="${user.phone }" class="ace"/><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;" id="ddddd">${vs.index+1}</td>
											<td class="center" style="cursor:pointer;color:blue;" onClick="toUserDetail('${user.userId}')">${user.username }—查看</td>
											<td class="center">${user.name }</td>
											<td class="center">
											
										
										<select class="chosen-select form-control" name="ROLE_ID" id="role_id"  style="vertical-align:top;width: 120px;">
											<c:forEach items="${roleList}" var="role">
												<option value="${role.ROLE_ID}" <c:if test="${user.roleId == role.ROLE_ID}"> selected </c:if> >${role.ROLE_NAME }</option>
												</c:forEach>
 	...
 	<script type="text/javascript">
		...
		$(top.hangge());
            function toUserDetail(uId){
			 ...
			 diag.Drag=true;
			 diag.Title ="用户账户";
			 diag.URL = '<%=basePath%>page/system/appuser/account';
			 diag.Width = 860;
			 diag.Height = 470;
