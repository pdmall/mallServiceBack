			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<div style="padding-top:8px;padding-left:1px;">
								<select id="tys">
									<option value="1">用户手机号：</option>
									<option value="2">用户名称：</option>
								
								</select><input type="text" id="sous" placeholder="这里输入关键字"/><a style="cursor:pointer;padding-left:3px" @click="sousuo()">搜索</a> <a @click="sousuos()"style="cursor:pointer;padding-left:20px;">刷新</a>
							</div>
							<table id="simple-table"
								...
								<thead>
									<tr>
										...
										<th class="center">介绍人名称</th>
										<th class="center">上级编号</th>
								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center' >{{ item.userIntegral }}</td>
											<td class='center' >{{ item.sname }}</td>
											<td class='center' >
											<span v-if="item.sjnumber==1">未进组</span>
											<span v-if="item.sjnumber!=1">{{item.sjnumber}}</span>
											
											</td>
											
											
											<td class='center'>
													<span v-if="item.roleId==1">普通用户</span>
													<span v-if="item.roleId==2">工作室负责人</span>
											</td>
											...
											<td class="center ">
												<div class="hidden-sm hidden-xs btn-group" id="dsdssd">
													<a class="btn btn-xs btn-success" title="编辑" @click="edit( item )">
													 ...
													 <a class="btn btn-xs btn-danger hidden"
														@click="delfff( item.userId )"> <i
							...
							<div class="page-header position-relative">
								<table style="width: 100%;">
									<tr>
										...
										<td class="hidden" style="vertical-align: top;  width: 140px;"> <a class="btn btn-mini btn-danger" @click="makeAll('确定要删除选中的数据吗?');"
												title="批量删除">
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>app/user/getappuserlist?page="+this.page+"&type="+this.type+"&keys="+this.keys;
	         ...
	         methods:{
	            ...
	            delfff:function(ids) {
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>app/user/getappuserlist?page="+this.cur+"&type="+this.type+"&keys="+this.keys;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>app/user/getappuserlist?page="+this.selecteType+"&type="+this.type+"&keys="+this.keys;
