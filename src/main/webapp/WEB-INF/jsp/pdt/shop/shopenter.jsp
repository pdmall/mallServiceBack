								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center'>{{item.legalname}}</td>
											...
											<td>
												<span v-if="item.type==1">未处理</span>
												<span v-if="item.type==2">已处理</span>
											</td>
											<td class='center'><a @click="xiang(item.id)" style="cursor:pointer">详情</a></td>
											
											<td class="center ">
												<div class="hidden-sm hidden-xs btn-group" id="dsdssd">
													<a class="btn btn-xs btn-success" v-if="item.type==1" title="处理" @click="change(item.id,2 )">
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>app/user/getshopenterlist?page="+this.page;
	         ...
	         methods:{
	        	 xiang:function(id){
	            	 ...
	            	 localStorage.setItem("shopenterid",id);
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="查看";
	    			 diag.URL = '<%=basePath%>page/pdt/shop/shopenterxiang';
	    			 diag.Width = 600;
	    			 diag.Height =350;
	        	 ...
	        	 change:function(id,status){
	        		...
	        		var url ="<%=basePath%>/app/user/updatedshopenter";	 
	        		$.ajax({
						type: "POST",
						url: url,
				    	data: {
	        	 ...
	        	 <!--添加用户-->
	      		add:function(){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="增加";
	    			 diag.URL = '<%=basePath%>/page/jmmr/producttype/producttypeadd';
	    			 diag.Width = 800;
	    			 diag.Height = 755;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            edit:function(item){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="编辑";
	    			 diag.URL = '<%=basePath%>page/jmmr/producttype/producttypeupdated';
	    			 diag.Width = 600;
	    			 diag.Height = 555;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/app/user/deleteshopenter";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {id:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>app/user/getshopenterlist?page="+this.cur;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>app/user/getshopenterlist?page="+this.selecteType;
		          ...
		          changeIsPause:function(selected){
		           ...
		           makeAll:function(){
   					...
   					var url = "<%=basePath%>/app/user/deleteproducts";
   					$.ajax({
							type: "POST",
							url: url,
