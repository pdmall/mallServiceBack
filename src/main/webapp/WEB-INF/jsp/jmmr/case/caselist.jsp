							...
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top: 5px;">
								...
								<tbody>
									...
									<tr v-for="item in items">
										...
										<td class='center' >{{ item.casename }}</td>
										<td class='center' >
											<span v-if="item.type==0">未审核</span>
											<span v-if="item.type==1">上级通过</span>
											<span v-if="item.type==2">上上级通过审核</span>
											<span v-if="item.type==3">上级未通过</span>
											<span v-if="item.type==3">上上级未通过，上级通过</span>
										</td>
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
			                ...
			                var url="<%=basePath%>app/user/getusercaselist?page="+this.page;
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/app/user/deletecase";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {ids:ids},
		          ...
		          changeType:function(pid){
		        	  ...
		        	  if(pid==0){
		        		  ...
		        		  var url="<%=basePath%>app/user/getusercaselist?page="+this.page+"&isPutaway="+this.selected+"&pId="+pid+"&shopId="+ids;
		        	  }else{
		        		  var url="<%=basePath%>app/user/getusercaselist?page="+this.page+"&isPutaway="+this.selected+"&pId="+pid;
		        	  }
		          ...
		          changeshopname:function(){
		           ...
		           makeAll:function(){
   					...
   					var url = "<%=basePath%>/app/user/deletecases";
   					$.ajax({
							type: "POST",
							url: url,
