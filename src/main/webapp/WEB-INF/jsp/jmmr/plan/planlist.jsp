							...
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top: 5px;">
								...
								<tbody>
									...
									<tr v-for="item in items">
										...
										<td class='center' >
											<span v-if="item.type==1">早上</span>
											<span v-if="item.type==2">中午</span>
											<span v-if="item.type==3">晚上</span>
										</td>
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
			                ...
			                var url="<%=basePath%>app/user/getplanlist?page="+this.page;
	         ...
	         methods:{
	            ...
	            edit:function(item){
	            	 localStorage.setItem("plansIds",item.id);
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="编辑";
	    			 diag.URL = '<%=basePath%>page/jmmr/plan/planupdated';
	    			 diag.Width = 500;
	    			 diag.Height = 355;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/app/user/deletePlan";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {ids:ids},
		          ...
		          changeType:function(pid){
		        	  ...
		        	  if(pid==0){
		        		  ...
		        		  var url="<%=basePath%>app/user/getplanlist?page="+this.page+"&isPutaway="+this.selected+"&pId="+pid+"&shopId="+ids;
		        	  }else{
		        		  var url="<%=basePath%>app/user/getplanlist?page="+this.page+"&isPutaway="+this.selected+"&pId="+pid;
		        	  }
		          ...
		          changeshopname:function(){
		           ...
		           makeAll:function(){
   					...
   					var url = "<%=basePath%>/app/user/deletePlans";
   					$.ajax({
							type: "POST",
							url: url,
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>page/jmmr/plan/planadd';
			 diag.Width = 500;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
