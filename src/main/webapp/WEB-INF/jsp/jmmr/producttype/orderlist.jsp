								...
								<div>
									<tbody>
										...
										<tr v-for="item in items">
											...
											<td class='center'><a @click="orderss(item.orderno)" style="cursor:pointer;">{{ item.orderno }}</a></td>
											<td class='center'>{{item.shopname}}</td>
											<td class='center'>{{item.name}}</td>
											<td class='center'>{{item.zcose/100}}</td>
											<td class='center'>
												<span v-if="item.status==0">未支付</span>
												<span v-if="item.status==1">已支付/待接单</span>
												<span v-if="item.status==2">待服务</span>
												<span v-if="item.status==3">服务中</span>
												<span v-if="item.status==4">已完成/待评价</span>
												<span v-if="item.status==5">已评价/待回复</span>
												<span v-if="item.status==6">已回复</span>
												<span v-if="item.status==7">用户已删除</span>
												<span v-if="item.status==8">已取消</span>
												<span v-if="item.status==9">已拒绝</span>
							...
							<div class="page-header position-relative">
								<table style="width: 100%;">
									<tr>
										...
										<td style="vertical-align: top; width: 140px;"> <a class="btn btn-mini btn-danger" @click="makeAll('确定要删除选中的数据吗?');"
												title="批量删除">
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>app/user/getorderlist?page="+this.page+"&status="+this.ss;
	         ...
	         methods:{
	        	 ...
	        	 orderss:function(orderno){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="查看";
	    			 diag.URL = '<%=basePath%>page/jmmr/producttype/orders';
	    			 diag.Width = 650;
	    			 diag.Height = 450;
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/app/user/deleteorderByid";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {id:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>app/user/getorderlist?page="+this.cur+"&status="+this.ss;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>app/user/getorderlist?page="+this.selecteType+"&status="+this.ss;
		          ...
		          changeIsPause:function(selected){
		           ...
		           makeAll:function(){
   					...
   					var url = "<%=basePath%>/app/user/deleteorderByids";
   					$.ajax({
							type: "POST",
							url: url,
