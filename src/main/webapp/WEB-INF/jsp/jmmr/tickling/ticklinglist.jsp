						...
						<div class="col-xs-12" id="pdtList">
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top: 5px;">
								...
								<tbody>
									...
									<tr v-for="item in items">
										...
										<td class='center' ><a class="btn btn-xs "  @click="photo(item.photo)">图片</a></td>
										...
										<td class="center">
											<div class="hidden-sm hidden-xs btn-group" id="dsdssd">
												<a class="btn btn-xs btn-danger" @click="edit( item.id )">
													详情
												</a>
											</div>
										</td>
							...
							<div class="page-header position-relative">
								<table style="width: 100%;">
									<tr>
										<td style="vertical-align: top; width: 140px;"><a class="btn btn-mini btn-success hidden" onclick="add();">新增</a> <a class="btn btn-mini btn-danger hidden" @click="makeAll('确定要删除选中的数据吗?');"
												title="批量删除">
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
			                ...
			                var url="<%=basePath%>app/user/getticklinglist?page="+this.page;
	         ...
	         methods:{
	            ...
	            edit:function(item){
	            	 localStorage.setItem("ticklingids",item);
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="详情";
	    			 diag.URL = '<%=basePath%>page/jmmr/tickling/tickling';
	    			 diag.Width = 600;
	    			 diag.Height = 555;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
		          ...
		          changeType:function(pid){
		        	  ...
		        	  if(pid==0){
		        		  ...
		        		  var url="<%=basePath%>app/user/getticklinglist?page="+this.page+"&isPutaway="+this.selected+"&pId="+pid+"&shopId="+ids;
		        	  }else{
		        		  var url="<%=basePath%>app/user/getticklinglist?page="+this.page+"&isPutaway="+this.selected+"&pId="+pid;
		        	  }
