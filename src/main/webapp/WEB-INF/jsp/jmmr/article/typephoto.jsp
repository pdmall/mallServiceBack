									...
									<tbody>
										<tr v-for="item in items">
											...
											<td class='center' >{{ item.name }}</td>
												<td style="padding-top: 10px;"><img :src="aa+item.photo" id="xhImgMin" width="100" height="100" style="margin-left: 223px; margin-top: 1px;"></td>
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group" id="dsdssd">
													<a class="btn btn-xs btn-success" title="编辑"
														...
														class="ace-icon fa fa-trash-o bigger-80" title="删除"></i>
													</a>
									...
									<tr>
										<td style="vertical-align: top; width: 140px;"><a
											class="btn btn-xs btn-success" title="增加" @click="add(  )">
												增加 </a> <a class="btn btn-mini btn-danger hidden"
											@click="makeAll('确定要删除选中的数据吗?');" title="批量删除"> <i
	...
	//showPdtList();
		...
		var example2 = new Vue({
			  ...
			  created: function(){
	                ...
	                var url="<%=basePath%>app/user/gettypephotolist?page="+this.page;
	         ...
	         methods:{
	        	 ...
	        	 <!--添加用户-->
	      		add:function(){
	    			 ...
	    			 diag.Drag=true;
	    			 diag.Title ="增加";
	    			 diag.URL = '<%=basePath%>page/jmmr/article/typephotoadd';
	    			 diag.Width = 600;
	    			 diag.Height = 655;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	         	...
	         	<!--修改弹出-->
	            edit:function(item){
	            	 localStorage.setItem("typephotoid",item.id);	            	 
	            	 localStorage.setItem("typephotoname",item.name);	            	 
	            	 localStorage.setItem("typephoto",item.photo);	            	 
	            	 top.jzts();
	    			 var diag = new top.Dialog();
	    			 diag.Drag=true;
	    			 diag.Title ="编辑";
	    			 diag.URL = '<%=basePath%>page/jmmr/article/typephotoupdated';
	    			 diag.Width = 600;
	    			 diag.Height = 655;
	    			 diag.Modal = true;				//有无遮罩窗口
	    			 diag. ShowMaxButton = true;	//最大化按钮
	            ...
	            delfff:function(ids) {
	            	bootbox.confirm("确定要删除吗?", function(result) {
	    				if(result) {
	    					top.jzts();
	    					var params={ids:ids}
	    					var url = "<%=basePath%>/app/user/deletetypephoto";
	    					$.ajax({
								type: "POST",
								url: url,
						    	data: {ids:ids},
	              ...
	              btnClick: function(data){//页码点击事件
		               ...
		               var url="<%=basePath%>advertising/gettypephotolist?page="+this.cur;
		          ...
		          changeType:function(pid){
		        	  ...
		        	  var url="<%=basePath%>advertising/gettypephotolist?page="+this.selecteType;
